...
void FujiDecompressor::fuji_compressed_load_raw()
{
    ...
    memcpy(block_sizes, mFile->getData(data_offset, sizeof (unsigned)*fuji_total_blocks), sizeof (unsigned)*fuji_total_blocks);
...
void FujiDecompressor::parse_fuji_compressed_header()
{
    ...
    const uchar8 *header = mFile->getData(data_offset, 16);
